...
public class AuCompare {
    ...
    protected void annotationComparison(AuStats stats, JCas jCas, List<String[]> rows) throws CASException {
            ...
            while(toolIndex < tList.size()) {
                ...
                if(au.getBegin() >= t.getEnd()) { //Au is to the right of the current tool
                    if(toolMatchIndex != toolIndex) {
                        ...
                        String i ="["; for(String r: statsClassList){i=i+";"+r;}
                        System.out.println("False Positive: " + documentID + i + "] " + tList.get(0));
                        tList.remove(0);
                    }
                    ...
                    if(toolIndex == toolMatchIndex) { toolMatchIndex++; }
                    toolIndex++;
        ...
        for(Annotation a : tList) { addRows(rows, statsClassList, stats, a); }
    }
     ...
     */
    protected void addRows(List<String[]> rows, String[] classList, AuStats auStats, Annotation...rowAnnotations) {
        if(rows == null) {
        ...
        if(rows == null) {
            throw new MissingResourceException("Missing required rows list", ArrayList.class.getCanonicalName(), "rows");
        }
        String begin, end, type, coveredText = null, mapping = auStats.getAuAnnotation() + "->" + auStats.getToolAnnotation();
